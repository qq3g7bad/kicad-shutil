...
# utils.sh - Utility functions for kicad-putil
...
http_get() {
    ...
    # Make HTTP request
    local response
    if response=$(curl -sS -L --max-time 15 \
        -A "Mozilla/5.0 (compatible; kicad-putil/1.0)" \
        "$url" 2>&1); then
...
http_check_url() {
    ...
    # Note: Using -s (silent) and no custom User-Agent to avoid server blocking
    http_code=$(curl -s -I -L --max-time 10 \
